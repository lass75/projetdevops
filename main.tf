resource "aws_security_group" "cicdSecGrp2" {
  name        = "CICD Security Group 2"
...
resource "aws_instance" "Jenkins" {
  ...
  key_name               = "key_devops"
  vpc_security_group_ids = [aws_security_group.cicdSecGrp2.id]
...
resource "aws_instance" "Prometheus-Grafana" {
  ...
  key_name               = "key_devops"
  vpc_security_group_ids = [aws_security_group.cicdSecGrp2.id]
  user_data              = templatefile("install_promotheus_grafana.sh", {})
  tags = {
